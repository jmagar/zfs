...
# shellcheck disable=SC2155
    ...
    # Add new cron entries with properly escaped paths. Checked: an unnoticed write
    # failure here would install a truncated crontab, silently dropping the schedules.
    if ! {
        ...
        echo "ERROR: Failed to write cron entries to $temp_crontab" >&2
        rm -f "$temp_crontab" "${temp_crontab}.new"
        return 1
    fi
